...
/** 扫描内容外部View的alpha值 */
static CGFloat const scanBorderOutsideViewAlpha = 0.7;
...
- (void)setupSubviews {
    ...
    scanContent_layer.borderColor = JPBaseColor.CGColor;//[[UIColor whiteColor] colorWithAlphaComponent:0.6].CGColor;
...
#pragma mark - - - 扫描外部View的创建
    ...
    promptLabel.text = @"扫描二维码完成商户入驻申请";
...
//        _scanningline.frame = CGRectMake(scanContent_X * 0.5, scanContent_Y, self.frame.size.width - scanContent_X - 10, JPRealValue(6));
        
        //  保证扫描指示线居中
        _scanningline.center = CGPointMake(self.frame.size.width / 2.0, scanContent_Y);
        _scanningline.bounds = CGRectMake(0, 0, self.frame.size.width - scanContent_X - 20, JPRealValue(6));
    }
