...
- (void)setupNavigationBar {
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:@"相册" style:(UIBarButtonItemStyleDone) target:self action:@selector(rightBarButtonItenAction)];
...
- (SGQRCodeScanningView *)scanningView {
    if (!_scanningView) {
        _scanningView = [SGQRCodeScanningView scanningViewWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - 64) layer:self.view.layer];
    }
...
- (void)readImageFromAlbum {
    ...
    if (device) {
        ...
        if (status == PHAuthorizationStatusNotDetermined) { // 用户还没有做出选择
            ...
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"温馨提示" message:JPCloseJurisdiction preferredStyle:(UIAlertControllerStyleAlert)];
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    if (detector) {
        ...
        if (features.count <= 0) {
            [SVProgressHUD showInfoWithStatus:@"未识别到有效的二维码"];
            return;
        }
